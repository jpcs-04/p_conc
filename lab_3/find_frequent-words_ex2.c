...
void *find_unique_words_letters(void *arg)
{
    ...
    for (int i = 0; i < n_total_words; i++)
    {
        ...
        if (isalpha(word_array[i][0]) && word_array[i][0] == current_letter)
        {
            word_info *list_aux = words_lists[current_letter - 'a'];
...
void threads_init_join()
{
    ...
    for (i = 0; i < 26; i++)
    {
        words_lists[i] = malloc(sizeof(word_info));
        words_lists[i]->next = NULL;
    }

    // Initialize threads
    for (i = 0; i < 26; i++)
    {
        char *thread_arg = malloc(sizeof(char));
        *thread_arg = letters[i];
        pthread_create(&threads[i], NULL, find_unique_words_letters, thread_arg);
    }
